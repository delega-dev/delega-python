...
class _AsyncTasksNamespace:
    ...
    async def complete(self, task_id: str, *, evidence: Optional[list[dict[str, Any]]] = None,
    ...
    async def request_human(self, task_id: str, *, criteria: list[str], expected_revision: int,
                            timeout_seconds: int = 1200) -> HumanRequest:
        """Register an immutable self-recipient checklist; does not run or send."""
        data = await self._http.post(request_path(task_id),
            body=registration_body(criteria, expected_revision, timeout_seconds))
        return HumanRequest.from_dict(data)

    async def human_request(self, task_id: str) -> HumanRequest:
        """Read request state and version without changing task ownership."""
        return HumanRequest.from_dict(await self._http.get(request_path(task_id)))

    async def human_result(self, task_id: str) -> HumanRequest:
        """Read server-verified completion; result is null until verified."""
        return HumanRequest.from_dict(await self._http.get(request_path(task_id) + "/result"))

    async def cancel_human_request(self, task_id: str, *, expected_version: int) -> HumanRequest:
        """Record cancellation without taking the executor's claim."""
        data = await self._http.post(request_path(task_id) + "/cancel",
